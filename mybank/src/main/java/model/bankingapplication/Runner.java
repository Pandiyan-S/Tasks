...
public class Runner
{
	...
	private void displayStatement(Map<Long,Transaction> statement)
	{
		...
		for(Entry<Long, Transaction> useage:statement.entrySet())
		{
			...
			logger.info("Transaction Id : " + useage.getKey());
			logger.info("Account number : " + useage.getValue().getAccountNumber());
			logger.info("Amount : " + useage.getValue().getAmount());
			logger.info("Type : " + useage.getValue().getModeOfTransaction());
			Instant point = Instant.ofEpochMilli(useage.getValue().getTime());
			LocalDateTime time = LocalDateTime.ofInstant(point, ZoneId.systemDefault());
			logger.info("Time of transaction : " + time);
			String toAcc = useage.getValue().getToAccount();
			if(toAcc != null)
			{
				logger.info("Transfer Account : " + toAcc);
			}
			logger.info("Credit or Debit : " + useage.getValue().getCreditOrDebit());
			logger.info("Approval : " + useage.getValue().getApproval());
		}
	}
			...
			if(logic.userType(userID).equalsIgnoreCase("customer"))
			{
				while (choice!=11)
				{
					...
					case 8:
						...
						Map<Long,Transaction> statement = perform.statement(userID,statementAccNo);
						run.displayStatement(statement);
						break;
					case 10:
						sc.nextLine();
						logger.info("Enter your Account number ");
						String activityAccNo = sc.nextLine();
						if(perform.checkCustomerAccountActive(userID, activityAccNo))
						{
							logger.info("The account is active ");
							break;
						}
						logger.info("The account is not active ");
						break;
					case 11:
