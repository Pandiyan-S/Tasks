...
public class LogicalLayer
{
	...
	public int findPos(PassengerInfo pass) throws UserException
	{
		StorageLayer store = new StorageLayer();
	...
	public int cancel(String pnr, String doj, String idNo) throws UserException
	{
		StorageLayer store = new StorageLayer();
		String path = "OnlineReservation/Tickets/"+doj+"/bookings.ser";
		Map<String,List<PassengerInfo>> hm = store.readFrom(path);
		String path2 = "OnlineReservation/Tickets/"+doj+"/rac.ser";
		Queue<PassengerInfo> rac = store.readList(path2);
		String path3 = "OnlineReservation/Tickets/"+doj+"/waitList.ser";
		Queue<PassengerInfo> waitList = store.readList(path3);
		int rno = 0;
		if(hm.containsKey(pnr))
		{
			...
			int seatNo = 0;
			String coach = "";
			List<PassengerInfo> list = hm.get(pnr);
			for(PassengerInfo pass: list)
			{
				if(pass.getIdNo().equals(idNo))
				{
					seatNo = pass.getSeatNo();
					coach = pass.getCoachClass();
					list.remove(pass);
					break;
				}
			}
			...
			if(seatNo == 0)
			{
				if(!waitList.isEmpty())
				{
					...
					Queue<PassengerInfo> newWaitList = new LinkedList<>();
					while(!waitList.isEmpty())
					{
						PassengerInfo pass = waitList.poll();
						Long now = System.currentTimeMillis();
						...
						if(pass.getPnr() == pnr)
						{
							if(pass.getIdNo() == idNo)
							{
								continue;
							}
						}
						newWaitList.add(pass);
					}
					if(newWaitList.size() != waitList.size())
					{
						store.writeList(path3, newWaitList);
					}
					else
					{
						return -1;
			...
			else
			{
				if(coach.equals("Sleeper"))
				{
					if(!rac.isEmpty())
					{
						PassengerInfo pass = rac.poll();
						store.writeList(path2, rac);
						rno = pass.getSeatNo();
						pass.setCoachClass("Sleeper");
						pass.setSeatNo(seatNo);
						List<PassengerInfo> temp = hm.get(pass.getPnr());
						for(PassengerInfo person: temp)
						{
							if(person.getIdNo() == pass.getIdNo())
							{
								temp.remove(person);
								temp.add(pass);
							}
						}
						hm.put(pass.getPnr(), temp);
						store.writeIn(path, hm);
					}
				}
				else
				...
				{
					PassengerInfo pass = waitList.poll();
					store.writeList(path3, waitList);
					pass.setCoachClass("RAC");
					pass.setSeatNo(rno);
					List<PassengerInfo> temp = new ArrayList<PassengerInfo>();
					if(hm.containsKey(pass.getPnr()))
					{
						temp = hm.get(pass.getPnr());
						temp.add(pass);
					}
					else
					{
